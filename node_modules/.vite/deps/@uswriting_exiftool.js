...
//#region node_modules/@6over3/zeroperl-ts/dist/esm/index.js
var { getPrototypeOf: ne, defineProperty: U, getOwnPropertyNames: oe } = Object;
((e) => typeof __require < "u" ? __require : typeof Proxy < "u" ? new Proxy(e, { get: (a, r) => (typeof __require < "u" ? __require : a)[r] }) : e)(function(e) {
...
var i = class i {
	...
	stringArraySize(e) {
		let a = e.length * 4, r = e.reduce((t, n) => t + this.byteLength(n) + 1, 0);
		return {
			pointerArraySize: a,
			bufferSize: r,
			totalSize: a + r
	...
	writeStringArray(e, a, r, t) {
		let n = r, o = t;
		for (let s of a) e.setUint32(n, o, !0), n += 4, o += this.writeString(e, `${s}\x00`, o);
		return o - t;
	}
	writeString(e, a, r) {
		let t = this.encoder.encode(a);
		return new Uint8Array(e.buffer, r, t.length).set(t), t.length;
	}
	readString(e, a, r) {
		let t = new Uint8Array(e.buffer, a, r);
	...
	iovViews(e, a, r) {
		let t = [], n = a;
		for (let o = 0; o < r; o++) {
			let s = e.getUint32(n + i.iovec_t.bufferOffset, !0), $ = e.getUint32(n + i.iovec_t.lengthOffset, !0);
			t.push(new Uint8Array(e.buffer, s, $)), n += i.iovec_t.size;
		}
		return t;
	}
	writeFilestat(e, a, r, t = 0n, n = 0n, o = 0n, s = 0n) {
		e.setBigUint64(a, 0n, !0), e.setBigUint64(a + 8, 0n, !0), e.setUint8(a + 16, r), e.setBigUint64(a + 24, 1n, !0), e.setBigUint64(a + 32, t, !0), e.setBigUint64(a + 40, n, !0), e.setBigUint64(a + 48, o, !0), e.setBigUint64(a + 56, s, !0);
	}
	writeFdstat(e, a, r, t, n, o) {
		e.setUint8(a, r), e.setUint16(a + 2, t, !0), e.setBigUint64(a + 8, n, !0), e.setBigUint64(a + 16, o, !0);
	}
};
var T$1 = class {
	code;
...
function z(e, a, r) {
	...
	return {
		args_get: (n, o) => {
			let s = r();
			return a.writeStringArray(s, t, n, o), i.WASI_ESUCCESS;
		},
		args_sizes_get: (n, o) => {
			let s = r();
			s.setUint32(n, t.length, !0);
			let $ = a.stringArraySize(t);
			return s.setUint32(o, $.bufferSize, !0), i.WASI_ESUCCESS;
		}
	};
}
function K(e, a, r) {
	return {
		clock_res_get: (t, n) => {
			let o;
			switch (t) {
				case i.WASI_CLOCK_MONOTONIC:
					o = 5e3;
					break;
				case i.WASI_CLOCK_REALTIME:
					o = 1e3;
					break;
				default: return i.WASI_ENOSYS;
			}
			return r().setUint32(n, o, !0), i.WASI_ESUCCESS;
		},
		clock_time_get: (t, n, o) => {
			let s = 0;
			switch (t) {
				case i.WASI_CLOCK_MONOTONIC:
					s = performance.now();
			...
			if (BigInt) {
				let E = BigInt(((k) => {
					let A = Math.trunc(k), l = BigInt(Math.round((k - A) * 1e6));
					return BigInt(A) * BigInt(1e6) + l;
				})(s));
				$.setBigUint64(o, E, !0);
			} else {
				let g = Date.now() * 1e6;
				$.setUint32(o, g & 65535, !0), $.setUint32(o + 4, g & 4294901760, !0);
			}
			return i.WASI_ESUCCESS;
		}
	};
}
function W$1(e, a, r) {
	return {
		environ_get: (t, n) => {
			...
			for (let g in e.env) {
				let E = e.env[g];
				$.setUint32(o, s, !0), o += 4, s += a.writeString($, `${g}=${E}\x00`, s);
			}
			return i.WASI_ESUCCESS;
		},
		environ_sizes_get: (t, n) => {
			let o = r();
			return o.setUint32(t, Object.keys(e.env || {}).length, !0), o.setUint32(n, Object.entries(e.env || {}).reduce((s, [$, g]) => {
				return s + a.byteLength($) + 1 + a.byteLength(g) + 1;
			}, 0), !0), i.WASI_ESUCCESS;
		}
	};
}
var D$1 = class {
	...
	constructor(e, a) {
		this.handler = e;
		this.outputBuffers = a;
	}
	writev(e) {
		let a = e.reduce((n, o) => n + o.byteLength, 0), r = 0, t = new Uint8Array(a);
		for (let n of e) t.set(n, r), r += n.byteLength;
		if (this.outputBuffers) this.handler(t);
		else {
			let n = this.decoder.decode(t);
			this.handler(n);
		}
		return t.length;
	}
...
var X = class {
	...
	consumePending(e, a) {
		if (e.byteLength < a) return this.pending = null, e;
		let r = e.slice(0, a);
		return this.pending = e.slice(a), r;
	}
	readv(e) {
		let a = 0;
		for (let r of e) {
			let t = r.byteLength;
			if (this.pending) {
				let n = this.consumePending(this.pending, t);
				r.set(n, 0), t -= n.byteLength, a += n.byteLength;
			}
			while (t > 0) {
				let n = this.consume(), o;
				if (n instanceof Uint8Array) o = n;
				else o = this.encoder.encode(n);
				if (o.length === 0) return a;
				if (o.length > t) r.set(o.slice(0, t), r.byteLength - t), this.pending = o.slice(t), a += t, t = 0;
				else r.set(o, r.byteLength - t), a += o.length, t -= o.length;
			}
		}
		return a;
	}
...
function le(e = {}) {
	let a = e.outputBuffers || !1;
	return [
		new X(e.stdin || (() => {
			return "";
		})),
		new D$1(e.stdout || console.log, a),
		new D$1(e.stderr || console.error, a)
	];
}
var M = class {
	root;
	...
	constructor(e) {
		if (this.root = {
			...
			type: "character",
			kind: "devnull"
		}), e) for (let a of Object.keys(e)) this.ensureDir(a), this.preopenPaths.push(a);
		else this.preopenPaths.push("/");
	}
	removeFile(e) {
		let r = this.normalizePath(e).split("/").filter((s) => s.length > 0), t = r.pop(), n = `/${r.join("/")}`, o = this.ensureDir(n);
		if (t) delete o.entries[t];
	}
	addFile(e, a) {
		if (typeof a === "string") {
			let r = new TextEncoder().encode(a);
			this.createFile(e, r);
			return;
		}
		this.createFile(e, a);
	...
	setNode(e, a) {
		let t = this.normalizePath(e).split("/").filter(($) => $.length > 0);
		if (t.length === 0) {
			if (a.type !== "dir") throw Error("Root must be a directory");
			this.root = a;
			return;
		}
		let n = t.pop(), o = `/${t.join("/")}`, s = this.ensureDir(o);
		if (n) s.entries[n] = a;
	}
	getDevNull() {
	...
	lookup(e) {
		let a = this.normalizePath(e);
		if (a === "/") return this.root;
		let r = a.split("/").filter((n) => n.length > 0), t = this.root;
		for (let n of r) {
			if (t.type !== "dir") return null;
			if (t = t.entries[n], !t) return null;
		}
		return t;
	}
	resolve(e, a) {
		let t = this.normalizePath(a).split("/").filter((o) => o.length > 0), n = e;
		for (let o of t) {
			if (o === ".") continue;
			if (o === "..") {
				n = this.root;
			...
			if (n = n.entries[o], !n) return null;
		}
		return n;
	}
	ensureDir(e) {
		let r = this.normalizePath(e).split("/").filter((n) => n.length > 0), t = this.root;
		for (let n of r) {
			if (!t.entries[n]) t.entries[n] = {
				type: "dir",
				entries: {}
			};
			let o = t.entries[n];
			if (o.type !== "dir") throw Error(`"${n}" is not a directory`);
			t = o;
		}
		return t;
	}
	createFileIn(e, a) {
		let t = this.normalizePath(a).split("/").filter(($) => $.length > 0);
		if (t.length === 0) throw Error("Cannot create a file with an empty name");
		let n = t.pop();
		if (!n) throw Error("Cannot create a file with an empty name");
		let o = e;
		for (let $ of t) {
			...
			let g = o.entries[$];
			if (g.type !== "dir") throw Error(`"${$}" is not a directory`);
			o = g;
		}
		let s = {
			type: "file",
		...
		return o.entries[n] = s, s;
	}
	normalizePath(e) {
		if (!e) return "/";
		let r = (e.startsWith("/") ? e : `/${e}`).replace(/\/+/g, "/");
		return r === "/" ? r : r.replace(/\/+$/, "");
	}
};
function R(e = {}) {
	return (a, r, t) => {
		let n = e.withFileSystem || new M(a.preopens), o = {};
		function s(A) {
			if (A.type === "file" && A.content instanceof Blob) {
				let l = BigInt(A.content.lastModified ?? Date.now()) * 1000000n;
				return {
					atim: l,
					mtim: l,
					ctim: l
		...
		let $ = 3;
		for (let A of n.getPreopenPaths()) {
			let l = n.lookup(A);
			if (l && l.type === "dir") o[$] = {
				node: l,
				position: 0,
		...
		function k(A) {
			if (A.content instanceof Blob) return A.content.size;
			return A.content.byteLength;
		}
		return {
			fd_read: async (A, l, b, m) => {
				let d = t(), f = r.iovViews(d, l, b), u = E(A);
				if (!u) return i.WASI_ERRNO_BADF;
				if (u.node.type === "character" && u.node.kind === "stdio") {
					let h = u.node.entry.readv(f);
					return d.setUint32(m, h, !0), i.WASI_ESUCCESS;
				}
				if (u.node.type === "dir") return i.WASI_ERRNO_ISDIR;
				if (u.node.type === "character" && u.node.kind === "devnull") return d.setUint32(m, 0, !0), i.WASI_ESUCCESS;
				let p = u.node, P = p.content, v = k(p) - u.position, F = 0;
				if (v <= 0) return d.setUint32(m, 0, !0), i.WASI_ESUCCESS;
				if (p.content instanceof Blob) {
					let h = p.content;
					for (let C of f) {
						if (u.position >= h.size) break;
						let S = Math.min(C.byteLength, h.size - u.position);
						if (S <= 0) break;
						let x = await h.slice(u.position, u.position + S).arrayBuffer();
						C.set(new Uint8Array(x)), F += x.byteLength, u.position += x.byteLength;
					}
				} else if (ArrayBuffer.isView(P)) for (let h of f) {
					if (u.position >= P.byteLength) break;
					let C = Math.min(h.byteLength, P.byteLength - u.position);
					if (C <= 0) break;
					h.set(P.slice(u.position, u.position + C)), F += C, u.position += C;
				}
				return d.setUint32(m, F, !0), i.WASI_ESUCCESS;
			},
			fd_write: (A, l, b, m) => {
				let d = t(), f = r.iovViews(d, l, b), u = E(A);
				if (!u) return i.WASI_ERRNO_BADF;
				let p = 0;
				if (u.node.type === "character" && u.node.kind === "stdio") {
					let C = u.node.entry.writev(f);
					return d.setUint32(m, C, !0), i.WASI_ESUCCESS;
				}
				if (u.node.type === "dir") return i.WASI_ERRNO_ISDIR;
				if (u.node.type === "character" && u.node.kind === "devnull") {
					let C = f.reduce((S, x) => S + x.byteLength, 0);
					return d.setUint32(m, C, !0), i.WASI_ESUCCESS;
				}
				if (u.node.content instanceof Blob) return i.WASI_ERRNO_INVAL;
				let P = u.position, v = f.reduce((C, S) => C + S.byteLength, 0), F = P + v, h;
				if (F > k(u.node)) h = new Uint8Array(F), h.set(u.node.content, 0);
				else h = u.node.content;
				for (let C of f) h.set(C, P), P += C.byteLength, p += C.byteLength;
				return u.node.content = h, u.position = P, d.setUint32(m, p, !0), i.WASI_ESUCCESS;
			},
			fd_close: (A) => {
				let l = E(A);
				if (!l) return i.WASI_ERRNO_BADF;
				if (l.node.type === "character" && l.node.kind === "stdio") return l.node.entry.close(), i.WASI_ESUCCESS;
				return delete o[A], i.WASI_ESUCCESS;
			},
			fd_seek: (A, l, b, m) => {
				let d = t(), f = E(A);
				if (!f) return i.WASI_ERRNO_BADF;
				if (f.node.type === "dir") return i.WASI_ERRNO_ISDIR;
				if (f.node.type === "character") return i.WASI_ERRNO_IO;
				let u = k(f.node), p;
				switch (b) {
					case 0:
						p = Number(l);
						break;
					case 1:
						p = f.position + Number(l);
						break;
					case 2:
						p = u + Number(l);
						break;
					default: return i.WASI_ERRNO_INVAL;
				}
				if (p < 0) return i.WASI_ERRNO_INVAL;
				return f.position = p, d.setBigUint64(m, BigInt(p), !0), i.WASI_ESUCCESS;
			},
			fd_tell: (A, l) => {
				let b = t(), m = E(A);
				if (!m) return i.WASI_ERRNO_BADF;
				if (m.node.type === "dir") return i.WASI_ERRNO_IO;
				if (m.node.type === "character") return i.WASI_ERRNO_IO;
				return b.setBigUint64(l, BigInt(m.position), !0), i.WASI_ESUCCESS;
			},
			fd_fdstat_get: (A, l) => {
				let b = t(), m = E(A);
				if (!m) return i.WASI_ERRNO_BADF;
				...
				switch (m.node.type) {
					case "character":
						d = i.WASI_FILETYPE_CHARACTER_DEVICE;
						break;
					case "dir":
						d = i.WASI_FILETYPE_DIRECTORY;
						break;
					case "file":
						d = i.WASI_FILETYPE_REGULAR_FILE;
						break;
				}
				let f = 536870911n;
				return r.writeFdstat(b, l, d, 0, f, f), i.WASI_ESUCCESS;
			},
			fd_filestat_get: (A, l) => {
				let b = t(), m = E(A);
				if (!m) return i.WASI_ERRNO_BADF;
				...
				switch (m.node.type) {
					...
					case "dir":
						...
						break;
					case "file":
						d = i.WASI_FILETYPE_REGULAR_FILE, f = k(m.node);
						break;
				}
				let { atim: u, mtim: p, ctim: P } = s(m.node);
				return r.writeFilestat(b, l, d, BigInt(f), u, p, P), i.WASI_ESUCCESS;
			},
			fd_prestat_get: (A, l) => {
				let b = t();
				if (A < 3) return i.WASI_ERRNO_BADF;
				let m = E(A);
				if (!m || !m.isPreopen) return i.WASI_ERRNO_BADF;
				b.setUint8(l, 0);
				let d = m.preopenPath || "";
				return b.setUint32(l + 4, d.length, !0), i.WASI_ESUCCESS;
			},
			fd_prestat_dir_name: (A, l, b) => {
				if (A < 3) return i.WASI_ERRNO_BADF;
				let m = E(A);
				if (!m || !m.isPreopen) return i.WASI_ERRNO_BADF;
				let d = m.preopenPath || "", f = t(), u = Math.min(d.length, b);
				for (let p = 0; p < u; p++) f.setUint8(l + p, d.charCodeAt(p));
				return i.WASI_ESUCCESS;
			},
			fd_open: (A, l, b, m, d, f, u, p) => {
				let P = t();
				if (A < 3) return i.WASI_ERRNO_NOTDIR;
				let v = E(A);
				if (!v || v.node.type !== "dir") return i.WASI_ERRNO_NOTDIR;
				let F = r.readString(P, l, b), h = (v.path.endsWith("/") ? v.path : `${v.path}/`) + F, C = g(h);
				if (C) return P.setUint32(p, C.fd, !0), i.WASI_ESUCCESS;
				let S = n.resolve(v.node, F), x = 1, I = 2, _ = 4;
				if (S) {
					if (m & I) return i.WASI_ERRNO_EXIST;
					if (m & _) {
						if (S.type !== "file") return i.WASI_ERRNO_INVAL;
						S.content = /* @__PURE__ */ new Uint8Array(0);
					}
				} else {
					if (!(m & x)) return i.WASI_ERRNO_NOENT;
					S = n.createFileIn(v.node, F);
				}
				return o[$] = {
					node: S,
					position: 0,
					...
					path: h,
					fd: $
				}, P.setUint32(p, $, !0), $++, i.WASI_ESUCCESS;
			},
			path_open: (A, l, b, m, d, f, u, p, P) => {
				let v = t();
				if (A < 3) return i.WASI_ERRNO_NOTDIR;
				let F = E(A);
				if (!F || F.node.type !== "dir") return i.WASI_ERRNO_NOTDIR;
				let h = r.readString(v, b, m), C = (F.path.endsWith("/") ? F.path : `${F.path}/`) + h, S = g(C);
				if (S) return v.setUint32(P, S.fd, !0), i.WASI_ESUCCESS;
				let x = n.resolve(F.node, h), I = 1, _ = 2, re = 4;
				if (x) {
					if (d & _) return i.WASI_ERRNO_EXIST;
					if (d & re) {
						if (x.type !== "file") return i.WASI_ERRNO_INVAL;
						x.content = /* @__PURE__ */ new Uint8Array(0);
					}
				} else {
					if (!(d & I)) return i.WASI_ERRNO_NOENT;
					x = n.createFileIn(F.node, h);
				}
				return o[$] = {
					node: x,
					position: 0,
					...
					path: C,
					fd: $
				}, v.setUint32(P, $, !0), $++, i.WASI_ESUCCESS;
			},
			path_filestat_get: (A, l, b, m, d) => {
				let f = t(), u = E(A);
				if (!u) return i.WASI_ERRNO_BADF;
				if (u.node.type !== "dir") return i.WASI_ERRNO_NOTDIR;
				let p = r.readString(f, b, m), P = u.path, v = P.endsWith("/") ? P + p : `${P}/${p}`, F = n.lookup(v);
				if (!F) return i.WASI_ERRNO_NOENT;
				if (F.type === "character" && F.kind === "stdio") return i.WASI_ERRNO_INVAL;
				let h, C = 0;
				if (F.type === "dir") h = i.WASI_FILETYPE_DIRECTORY;
				else if (F.type === "character" && F.kind === "devnull") h = i.WASI_FILETYPE_CHARACTER_DEVICE;
				else h = i.WASI_FILETYPE_REGULAR_FILE, C = k(F);
				let { atim: S, mtim: x, ctim: I } = s(F);
				return r.writeFilestat(f, d, h, BigInt(C), S, x, I), i.WASI_ESUCCESS;
			}
		};
	};
}
...
function J(e, a, r) {
	return { random_get: (t, n) => {
		let o = r(), s = new Uint8Array(o.buffer, t, n);
		return crypto.getRandomValues(s), i.WASI_ESUCCESS;
	} };
}
var G = class {
	wasiImport;
	...
	abi;
	constructor(e) {
		if (this.wasiImport = {}, this.abi = new i(), e?.features) {
			let a = {};
			for (let r of e.features) {
				let t = r.name || "Unknown feature", n = r(e, this.abi, this.view.bind(this));
				for (let o in n) {
					if (o in this.wasiImport) {
						let s = a[o] || "Unknown feature";
						throw Error(`Import conflict: Function '${o}' is already provided by '${s}' and is being redefined by '${t}'`);
					}
					a[o] = t;
				}
				this.wasiImport = {
		...
		for (let a of i.IMPORT_FUNCTIONS) if (!(a in this.wasiImport)) this.wasiImport[a] = () => {
			return i.WASI_ENOSYS;
		};
	}
	get exports() {
	...
	async start(e) {
		...
		try {
			return await this.instance.exports._start(), i.WASI_ESUCCESS;
		} catch (a) {
			if (a instanceof T$1) return a.code;
			throw a;
		}
	}
};
var B = /* @__PURE__ */ new WeakMap();
var $e = /* @__PURE__ */ new Set(["free", "malloc"]);
function ue(e) {
	return !!e && (typeof e === "object" || typeof e === "function") && typeof e.then === "function";
}
function Z(e, a) {
	return new Proxy(e, { get: (r, t) => a(r[t]) });
}
var L = class {
	value = void 0;
	exports = null;
	unwrappedExports;
	constructor(e) {
		this.unwrappedExports = /* @__PURE__ */ new Set([...$e, ...e?.unwrappedExports ?? []]);
	}
	getState() {
	...
	wrapModuleImports(e) {
		return Z(e, (a) => {
			if (typeof a === "function") return this.wrapImportFn(a);
			return a;
		});
	}
	wrapImports(e) {
		if (e === void 0) return;
		return Z(e, (a = Object.create(null)) => this.wrapModuleImports(a));
	}
	wrapExportFn(e) {
		let a = B.get(e);
		...
		return a = async (...r) => {
			...
			while (this.getState() === 1) {
				if (!this.exports) throw Error("Exports not initialized");
				this.exports.asyncify_stop_unwind(), this.value = await this.value, this.assertNoneState(), this.exports.asyncify_start_rewind(16), t = e(...r);
			}
			return this.assertNoneState(), t;
		}, B.set(e, a), a;
	}
	wrapExports(e) {
		...
		for (let r in e) {
			let t = e[r];
			if (typeof t === "function" && !r.startsWith("asyncify_") && !this.unwrappedExports.has(r)) t = this.wrapExportFn(t);
	...
	init(e, a) {
		let r = e.exports, t = r.memory || a?.env && a.env.memory;
		if (!t) throw Error("Memory not found in exports or imports.env");
		let n;
		if (r.__stack_pointer) n = r.__stack_pointer.value;
		else n = 1024;
		new Int32Array(t.buffer, 16).set([24, n]), this.exports = this.wrapExports(r), Object.setPrototypeOf(e, O.prototype);
	}
};
var O = class extends WebAssembly.Instance {
	constructor(e, a, r) {
		let t = new L(r);
		super(e, t.wrapImports(a));
		t.init(this, a);
	}
	get exports() {
		return B.get(super.exports);
	}
};
Object.defineProperty(O.prototype, "exports", { enumerable: !0 });
async function Y(e, a, r) {
	let t = new L(r), n = await WebAssembly.instantiate(e, t.wrapImports(a));
	return t.init(n.instance, a), n;
}
var q = "./zeroperl.wasm";
var de = [
	...
	"zeroperl_array_get",
	"zeroperl_array_set",
	"zeroperl_array_length",
	"zeroperl_array_clear",
	"zeroperl_array_to_value",
	"zeroperl_value_to_array",
	"zeroperl_array_free",
	"zeroperl_new_hash",
	"zeroperl_hash_set",
	"zeroperl_hash_get",
	"zeroperl_hash_exists",
	"zeroperl_hash_delete",
	"zeroperl_hash_clear",
	"zeroperl_hash_iter_new",
	"zeroperl_hash_iter_next",
	"zeroperl_hash_iter_free",
	"zeroperl_hash_to_value",
	"zeroperl_value_to_hash",
	"zeroperl_hash_free",
	"zeroperl_new_ref",
	"zeroperl_deref",
	"zeroperl_is_ref",
	"zeroperl_get_var",
	"zeroperl_get_array_var",
	"zeroperl_get_hash_var",
	"zeroperl_set_var",
	"zeroperl_register_function",
	"zeroperl_register_method",
	"zeroperl_result_get",
	"zeroperl_result_free",
	"zeroperl_set_host_error",
	"zeroperl_get_host_error",
	"zeroperl_clear_host_error"
];
var c$1 = class c$1 extends Error {
	exitCode;
	perlError;
	constructor(e, a, r) {
		super(e);
		if (this.name = "ZeroPerlError", this.exitCode = a, this.perlError = r, Error.captureStackTrace) Error.captureStackTrace(this, c$1);
	}
};
...
async function ce(e) {
	...
	let a;
	if (Ee()) a = await (await (e ?? fetch)(q)).arrayBuffer();
	else {
		let t = new URL(q, import.meta.url).pathname;
		if (typeof Deno < "u") a = (await Deno.readFile(t)).buffer;
		else if (typeof Bun < "u") a = await Bun.file(t).arrayBuffer();
		else {
			let { readFile: n } = await import("./promises-BLBYDbiM.js").then((m) => /* @__PURE__ */ __toESM(m.default));
			a = (await n(t)).buffer;
		}
	}
	return w = new WeakRef(a), a;
}
function fe(e) {
	return [
		"undef",
...
var y$1 = class y$1 {
	...
	toInt() {
		...
		try {
			if (!this.exports.zeroperl_to_int(this.ptr, e)) throw new c$1("Failed to convert value to int");
	...
	toDouble() {
		...
		try {
			if (!this.exports.zeroperl_to_double(this.ptr, e)) throw new c$1("Failed to convert value to double");
	...
	toString() {
		...
		let e = this.exports.malloc(4);
		try {
			let a = this.exports.zeroperl_to_string(this.ptr, e);
			if (a === 0) return "";
			let r = new DataView(this.exports.memory.buffer).getUint32(e, !0);
			return V.decode(new Uint8Array(this.exports.memory.buffer, a, r));
		} finally {
			this.exports.free(e);
		}
	}
	...
	getType() {
		return this.checkDisposed(), fe(this.exports.zeroperl_get_type(this.ptr));
	}
	project() {
	...
	createRef() {
		...
		if (e === 0) throw new c$1("Failed to create reference");
		return new y$1(e, this.exports);
	}
	deref() {
		this.checkDisposed();
		let e = this.exports.zeroperl_deref(this.ptr);
		if (e === 0) throw new c$1("Failed to dereference value");
		return new y$1(e, this.exports);
	}
	incref() {
	...
	dispose() {
		...
		this.exports.zeroperl_value_free(this.ptr), this.disposed = !0;
	}
	checkDisposed() {
		if (this.disposed) throw new c$1("PerlValue has been disposed");
	}
};
var N = class N {
	ptr;
	...
	disposed = !1;
	constructor(e, a, r) {
		this.ptr = e, this.exports = a, this.perl = r;
	}
	getPtr() {
		return this.checkDisposed(), this.ptr;
	}
	push(e) {
		this.checkDisposed();
		let a = this.perl.toPerlValue(e);
		try {
			this.exports.zeroperl_array_push(this.ptr, a.getPtr());
		} finally {
			if (!(e instanceof y$1)) a.dispose();
		}
	}
	pop() {
		this.checkDisposed();
		let e = this.exports.zeroperl_array_pop(this.ptr);
		return e === 0 ? null : new y$1(e, this.exports);
	}
	get(e) {
		this.checkDisposed();
		let a = this.exports.zeroperl_array_get(this.ptr, e);
		return a === 0 ? null : new y$1(a, this.exports);
	}
	set(e, a) {
		this.checkDisposed();
		let r = this.perl.toPerlValue(a);
		try {
			if (!this.exports.zeroperl_array_set(this.ptr, e, r.getPtr())) throw new c$1(`Failed to set array element at index ${e}`);
		} finally {
			if (!(a instanceof y$1)) r.dispose();
		}
	}
	getLength() {
	...
	toValue() {
		...
		if (e === 0) throw new c$1("Failed to convert array to value");
		return new y$1(e, this.exports);
	}
	project() {
		this.checkDisposed();
		let e = this.getLength(), a = [];
		for (let r = 0; r < e; r++) {
			let t = this.get(r);
			if (t) a.push(t.project()), t.dispose();
			else a.push(null);
		}
		return a;
	}
	static fromValue(e, a) {
		let r = e.exports, t = r.zeroperl_value_to_array(e.getPtr());
		return t === 0 ? null : new N(t, r, a);
	}
	*[Symbol.iterator]() {
	...
	dispose() {
		if (this.disposed) return;
		this.exports.zeroperl_array_free(this.ptr), this.disposed = !0;
	}
	checkDisposed() {
		if (this.disposed) throw new c$1("PerlArray has been disposed");
	}
};
var j = class j {
	ptr;
	...
	set(e, a) {
		this.checkDisposed();
		let r = this.perl.toPerlValue(a), t = this.writeCString(e);
		try {
			if (!this.exports.zeroperl_hash_set(this.ptr, t, r.getPtr())) throw new c$1(`Failed to set hash key '${e}'`);
		} finally {
			if (this.exports.free(t), !(a instanceof y$1)) r.dispose();
		}
	}
	get(e) {
		this.checkDisposed();
		let a = this.writeCString(e);
		try {
			let r = this.exports.zeroperl_hash_get(this.ptr, a);
			return r === 0 ? null : new y$1(r, this.exports);
		} finally {
			this.exports.free(a);
		}
	}
	has(e) {
		...
		let a = this.writeCString(e);
		try {
			return this.exports.zeroperl_hash_exists(this.ptr, a) !== 0;
		} finally {
			this.exports.free(a);
		}
	}
	delete(e) {
		...
		let a = this.writeCString(e);
		try {
			return this.exports.zeroperl_hash_delete(this.ptr, a) !== 0;
		} finally {
			this.exports.free(a);
		}
	}
	...
	toValue() {
		...
		if (e === 0) throw new c$1("Failed to convert hash to value");
		return new y$1(e, this.exports);
	}
	project() {
		this.checkDisposed();
		let e = {};
		for (let [a, r] of this.entries()) e[a] = r.project(), r.dispose();
		return e;
	}
	static fromValue(e, a) {
		let r = e.exports, t = r.zeroperl_value_to_hash(e.getPtr());
		return t === 0 ? null : new j(t, r, a);
	}
	*entries() {
		this.checkDisposed();
		let e = this.exports.zeroperl_hash_iter_new(this.ptr);
		if (e === 0) throw new c$1("Failed to create hash iterator");
		let a = this.exports.malloc(4), r = this.exports.malloc(4);
		try {
			while (this.exports.zeroperl_hash_iter_next(e, a, r)) {
				let t = new DataView(this.exports.memory.buffer), n = t.getUint32(a, !0), o = t.getUint32(r, !0);
				yield [this.readCString(n), new y$1(o, this.exports)];
			}
		} finally {
			this.exports.free(a), this.exports.free(r), this.exports.zeroperl_hash_iter_free(e);
		}
	}
	*keys() {
		for (let [e, a] of this.entries()) a.dispose(), yield e;
	}
	*values() {
	...
	dispose() {
		if (this.disposed) return;
		this.exports.zeroperl_hash_free(this.ptr), this.disposed = !0;
	}
	writeCString(e) {
		let a = H.encode(`${e}\x00`), r = this.exports.malloc(a.length);
		return new Uint8Array(this.exports.memory.buffer).set(a, r), r;
	}
	readCString(e) {
		if (e === 0) return "";
		let a = new Uint8Array(this.exports.memory.buffer), r = 0;
		while (a[e + r] !== 0) r++;
		return V.decode(a.subarray(e, e + r));
	}
	checkDisposed() {
		if (this.disposed) throw new c$1("PerlHash has been disposed");
	}
};
var ae = class ae {
	wasi;
	...
	static async create(e = {}) {
		let a = await ce(e.fetch), r = e.fileSystem || new M({ "/": "" }), n = new G({
			env: e.env || {},
			args: ["zeroperl"],
			features: [
				W$1,
		...
		}), o = new ae(n), s = async (E, k, A) => o.handleHostCall(E, k, A), { instance: $ } = await Y(a, {
			wasi_snapshot_preview1: n.wasiImport,
			env: { call_host_function: s }
		}, { unwrappedExports: de });
		await n.initialize($);
		let g = await o.exports.zeroperl_init();
		if (g !== 0) throw new c$1("Failed to initialize Perl interpreter", g, o.getLastError());
		return o;
	}
	async handleHostCall(e, a, r) {
		let t = this.hostFunctions.get(e);
		if (!t) return this.setHostError(`Host function ${e} not found`), 0;
		try {
			let n = [];
			if (a > 0) {
				let $ = new DataView(this.exports.memory.buffer);
				for (let g = 0; g < a; g++) {
					let E = $.getUint32(r + g * 4, !0);
					if (E !== 0) n.push(new y$1(E, this.exports));
				}
			}
			let o = await t(...n);
			if (o instanceof y$1) return o.getPtr();
			let s = this.exports.zeroperl_new_undef();
			if (s === 0) return this.setHostError("Failed to allocate return value"), 0;
			return s;
		} catch (n) {
			return this.setHostError(n instanceof Error ? n.message : String(n)), 0;
		}
	}
	setHostError(e) {
		let a = this.writeCString(e);
		if (a) this.exports.zeroperl_set_host_error(a), this.exports.free(a);
	}
	createInt(e) {
		this.checkDisposed();
		let a = this.exports.zeroperl_new_int(Math.floor(e));
		if (a === 0) throw new c$1("Failed to create integer value");
		return new y$1(a, this.exports);
	}
	createUInt(e) {
		this.checkDisposed();
		let a = this.exports.zeroperl_new_uint(Math.floor(Math.abs(e)));
		if (a === 0) throw new c$1("Failed to create unsigned integer value");
		return new y$1(a, this.exports);
	}
	createDouble(e) {
		this.checkDisposed();
		let a = this.exports.zeroperl_new_double(e);
		if (a === 0) throw new c$1("Failed to create double value");
		return new y$1(a, this.exports);
	}
	createString(e) {
		this.checkDisposed();
		let a = H.encode(e), r = this.exports.malloc(a.length);
		new Uint8Array(this.exports.memory.buffer).set(a, r);
		try {
			let t = this.exports.zeroperl_new_string(r, a.length);
			if (t === 0) throw new c$1("Failed to create string value");
			return new y$1(t, this.exports);
		} finally {
			this.exports.free(r);
		}
	}
	createBool(e) {
		this.checkDisposed();
		let a = this.exports.zeroperl_new_bool(e ? 1 : 0);
		if (a === 0) throw new c$1("Failed to create boolean value");
		return new y$1(a, this.exports);
	}
	createUndef() {
		this.checkDisposed();
		let e = this.exports.zeroperl_new_undef();
		if (e === 0) throw new c$1("Failed to create undef value");
		return new y$1(e, this.exports);
	}
	createArray(e) {
		this.checkDisposed();
		let a = this.exports.zeroperl_new_array();
		if (a === 0) throw new c$1("Failed to create array");
		let r = new N(a, this.exports, this);
		if (e) for (let t of e) r.push(t);
		return r;
	}
	createHash(e) {
		this.checkDisposed();
		let a = this.exports.zeroperl_new_hash();
		if (a === 0) throw new c$1("Failed to create hash");
		let r = new j(a, this.exports, this);
		if (e) for (let [t, n] of Object.entries(e)) r.set(t, n);
		return r;
	}
	toPerlValue(e) {
		if (e instanceof y$1) return e;
		...
		if (Array.isArray(e)) {
			let a = this.createArray(e), r = a.toValue();
			return a.dispose(), r;
		}
		if (typeof e === "object") {
			let a = this.createHash(e), r = a.toValue();
			return a.dispose(), r;
		}
		throw new c$1(`Cannot convert value of type ${typeof e} to PerlValue`);
	}
	getVariable(e) {
		this.checkDisposed();
		let a = this.writeCString(e);
		try {
			let r = this.exports.zeroperl_get_var(a);
			return r === 0 ? null : new y$1(r, this.exports);
		} finally {
			this.exports.free(a);
		}
	...
	getArrayVariable(e) {
		...
		let a = this.writeCString(e);
		try {
			let r = this.exports.zeroperl_get_array_var(a);
	...
	getHashVariable(e) {
		...
		let a = this.writeCString(e);
		try {
			let r = this.exports.zeroperl_get_hash_var(a);
			return r === 0 ? null : new j(r, this.exports, this);
		} finally {
			this.exports.free(a);
		}
	}
	setVariable(e, a) {
		this.checkDisposed();
		let r = this.toPerlValue(a), t = this.writeCString(e);
		try {
			if (!this.exports.zeroperl_set_var(t, r.getPtr())) throw new c$1(`Failed to set variable '${e}'`);
		} finally {
			if (this.exports.free(t), !(a instanceof y$1)) r.dispose();
		}
	}
	registerFunction(e, a) {
		this.checkDisposed();
		let r = this.nextFuncId++;
		this.hostFunctions.set(r, a);
		let t = this.writeCString(e);
		try {
			this.exports.zeroperl_register_function(r, t);
		} finally {
			this.exports.free(t);
		}
	}
	registerMethod(e, a, r) {
		this.checkDisposed();
		let t = this.nextFuncId++;
		this.hostFunctions.set(t, r);
		let n = this.writeCString(e), o = this.writeCString(a);
		try {
			this.exports.zeroperl_register_method(t, n, o);
		} finally {
			this.exports.free(n), this.exports.free(o);
		}
	}
	async call(e, a = [], r = "scalar") {
		this.checkDisposed();
		let t = this.writeCString(e), n = ee(r), o = 0;
		if (a.length > 0) {
			o = this.exports.malloc(a.length * 4);
			let s = new DataView(this.exports.memory.buffer);
			for (let $ = 0; $ < a.length; $++) {
				let g = a[$];
				if (!g) throw new c$1(`Argument at index ${$} is undefined`);
				s.setUint32(o + $ * 4, g.getPtr(), !0);
			}
		}
		try {
			let s = await this.exports.zeroperl_call(t, n, a.length, o);
			if (s === 0) {
				if (r === "void") return;
				if (r === "scalar") return null;
				return [];
			}
			let $ = new DataView(this.exports.memory.buffer), g = $.getInt32(s, !0), E = [];
			for (let A = 0; A < g; A++) {
				let l = this.exports.zeroperl_result_get(s, A);
				if (l !== 0) E.push(new y$1(l, this.exports));
			}
			let k = $.getUint32(s + 4, !0);
			if (k !== 0) this.exports.free(k);
			if (this.exports.free(s), r === "void") {
				for (let A of E) A.dispose();
				return;
			}
			if (r === "scalar") return E[0] ?? null;
			return E;
		} catch (s) {
			if (s instanceof T$1) {
				if (r === "void") return;
			...
			if (this.exports.free(t), o !== 0) this.exports.free(o);
		}
	}
	async eval(e, a = []) {
		this.checkDisposed();
		let r = this.writeCString(e), t = 0, n = [];
		if (a.length > 0) {
			let o = this.writeStringArray(a);
			t = o.argv, n = o.buffers;
		}
		try {
			let o = await this.exports.zeroperl_eval(r, ee("scalar"), a.length, t);
			if (o !== 0) return {
				success: !1,
			...
			if (this.exports.free(r), n.length > 0) this.freeStringArray(t, n);
		}
	}
	async runFile(e, a = []) {
		this.checkDisposed();
		...
		try {
			let o = await this.exports.zeroperl_run_file(r, a.length, t);
			if (o !== 0) return {
				success: !1,
				error: this.getLastError(),
				exitCode: o
	...
	async reset() {
		...
		if (e !== 0) throw new c$1("Failed to reset Perl interpreter", e, this.getLastError());
	}
	flush() {
		if (this.checkDisposed(), this.exports.zeroperl_flush() !== 0) throw new c$1("Failed to flush output buffers");
	}
	getLastError() {
	...
	dispose() {
		...
		this.exports.zeroperl_free_interpreter(), this.isDisposed = !0, this.hostFunctions.clear();
	}
	shutdown() {
		if (this.isDisposed) return;
		this.exports.zeroperl_shutdown(), this.isDisposed = !0, this.hostFunctions.clear();
	}
	writeCString(e) {
		if (!e) return 0;
		let a = H.encode(`${e}\x00`), r = this.exports.malloc(a.length);
	...
	writeStringArray(e) {
		let a = [], r = this.exports.malloc(e.length * 4), t = new DataView(this.exports.memory.buffer);
		for (let n = 0; n < e.length; n++) {
			let o = e[n];
			if (o === void 0) throw new c$1(`Argument at index ${n} is undefined`);
			let s = this.writeCString(o);
			a.push(s), t.setUint32(r + n * 4, s, !0);
		}
		return {
			argv: r,
			buffers: a
...
`), this;
	...
	static isMultiline($) {
		let t = 0;
		for (let e = 0; e < $.length; e++) if ($[e] === `
`) t++;
		else if ($[e] === "\r" && (e === $.length - 1 || $[e + 1] !== `
`)) t++;
		return t > 0;
	}
};
...
async function T($) {
	let t = g?.deref(), e = x?.deref();
	if (t && e) return {
		perl: t,
		fileSystem: e
	};
	return e = new M({ "/": "" }), e.addFile("/exiftool", y), t = await ae.create({
		fileSystem: e,
		stdout: (i) => {
			let n = typeof i === "string" ? i : F.decode(i);
			c.append(n);
		},
		stderr: (i) => {
			let n = typeof i === "string" ? i : F.decode(i);
		...
		fetch: $
	}), g = new WeakRef(t), x = new WeakRef(e), {
		perl: t,
		fileSystem: e
	};
}
...
function h($) {
	return Object.entries($).flatMap(([t, e]) => Array.isArray(e) ? e.map((i) => `-${t}=${i}`) : [`-${t}=${e}`]);
}
async function D($, t = {}) {
	let { perl: e, fileSystem: i } = await T(t.fetch), n = [];
	c.clear(), m.clear(), await e.reset();
	try {
		let a = `/${$.name}`;
		if ($ instanceof File) i.addFile(a, $);
		else i.addFile(a, $.data);
		n.push(a);
		let l = [...t.args || []];
		if (t.config) {
			let r = `/${t.config.name}`;
			if (t.config instanceof File) i.addFile(r, t.config);
			else i.addFile(r, t.config.data);
			n.push(r), l.push("-config", r);
		}
		l.push(a);
		let s = await e.runFile("/exiftool", l);
		e.flush();
		let o = m.toString(), f = c.toString();
		if (!s.success || s.exitCode !== 0) return {
			success: !1,
			data: void 0,
			error: e.getLastError() || o || "Unknown error",
			exitCode: s.exitCode
		};
		if (o && o.trim()) return {
			success: !1,
		...
		let d;
		if (t.transform) d = t.transform(f);
		else d = f;
		return {
			success: !0,
			data: d,
...
async function W($, t, e = {}) {
	let { perl: i, fileSystem: n } = await T(e.fetch), a = [];
	c.clear(), m.clear(), await i.reset();
	try {
		let l = `/${$.name}`;
		if ($ instanceof File) n.addFile(l, $);
		else n.addFile(l, $.data);
		a.push(l);
		let s = [...e.args || []];
		if (e.config) {
			let u = `/${e.config.name}`;
			if (e.config instanceof File) n.addFile(u, e.config);
			else n.addFile(u, e.config.data);
			a.push(u), s.push("-config", u);
		}
		s.push(...h(t));
		let o = `/${crypto.randomUUID().replace(/-/g, "")}.tmp`;
		a.push(o), s.push("-o", o), s.push(l);
		let f = await i.runFile("/exiftool", s);
		i.flush();
		let d = m.toString();
		if (!f.success || f.exitCode !== 0) return {
			success: !1,
			data: void 0,
			error: i.getLastError() || d || "Unknown error",
			exitCode: f.exitCode
		};
		if (d && d.trim()) return {
			success: !1,
		...
		if (!r || r.type !== "file") return {
			success: !1,
			data: void 0,
			error: `Temporary output file not found: ${o}`,
			exitCode: 0
		};
		return {
			success: !0,
			data: r.content instanceof Blob ? await r.content.arrayBuffer() : r.content.buffer,
			exitCode: 0
		};
	} finally {
		v(n, a);
...
async function _() {
	let $ = g?.deref();
	if ($) $.dispose(), g = null, x = null;
}
//#endregion
export { _ as dispose, D as parseMetadata, W as writeMetadata };
