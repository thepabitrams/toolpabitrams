// src/tools/houseandhome/tenant-billing/modules/owner/OwnerChargesCard/MeterCharge.tsx
...
export const MeterCharge: React.FC<MeterChargeProps> = ({
  ...
  const [totalBill, setTotalBill] = useState(charge.totalBill ?? 0);
  const [totalUnits, setTotalUnits] = useState(charge.totalUnits ?? 0);
  const [note, setNote] = useState(charge.note ?? '');
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    setTotalBill(charge.totalBill ?? 0);
    setTotalUnits(charge.totalUnits ?? 0);
    setNote(charge.note ?? '');
  }, [charge]);

  const rate = totalUnits > 0 ? totalBill / totalUnits : 0;
  ...
  }, [totalBill, totalUnits, rate, note]);
  ...
  const tenantCount = charge.applyTo === 'all' 
    ? 'All tenants' 
    : `${charge.selectedTenants?.length || 0} tenants selected`;

  const handleCopy = async () => {
    const text = `${charge.name}\nTotal Bill: $${totalBill.toFixed(2)}\nTotal Units: ${totalUnits}\nPer Unit Rate: $${rate.toFixed(4)}`;
    ...
    <Card className="p-4 shadow-sm hover:shadow-md transition-all duration-200 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
      ...
      <div className="space-y-2 mt-2">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs text-gray-500 block">Total Bill ($)</label>
            <Input type="number" step="0.01" value={totalBill} onChange={(e) => setTotalBill(parseFloat(e.target.value) || 0)} className={`w-full text-sm ${isReadOnly ? 'bg-gray-100 dark:bg-gray-700/50 cursor-not-allowed opacity-60' : ''}`} disabled={isReadOnly} placeholder="0.00" />
          </div>
          <div>
            <label className="text-xs text-gray-500 block">Total Units</label>
            <Input type="number" step="0.01" value={totalUnits} onChange={(e) => setTotalUnits(parseFloat(e.target.value) || 0)} className={`w-full text-sm ${isReadOnly ? 'bg-gray-100 dark:bg-gray-700/50 cursor-not-allowed opacity-60' : ''}`} disabled={isReadOnly} placeholder="0" />
          </div>
        </div>

        {totalUnits > 0 && (
          <div className="text-sm text-gray-500">Per Unit Rate: <span className="font-medium text-gray-700">${rate.toFixed(4)}</span></div>
        )}
