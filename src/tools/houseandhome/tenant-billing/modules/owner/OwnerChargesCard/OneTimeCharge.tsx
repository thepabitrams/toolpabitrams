// src/tools/houseandhome/tenant-billing/modules/owner/OwnerChargesCard/OneTimeCharge.tsx
...
export const OneTimeCharge: React.FC<OneTimeChargeProps> = ({
  charge,
  onUpdate,
  onEdit,
  onTogglePaid,
  ...
  const [totalAmount, setTotalAmount] = useState(charge.totalAmount ?? 0);
  const [note, setNote] = useState(charge.note ?? '');
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    setTotalAmount(charge.totalAmount ?? 0);
    setNote(charge.note ?? '');
  }, [charge]);

  const splitCount = charge.splitAmong?.length || 0;
  const eachPays = splitCount > 0 ? totalAmount / splitCount : 0;

  useEffect(() => {
    onUpdate({ ...charge, totalAmount, eachPays, note });
  }, [totalAmount, eachPays, note]);
  ...
  const handleCopy = async () => {
    ...
    if (splitCount > 0) text += `\nSplit among: ${splitCount} tenants\nEach pays: $${eachPays.toFixed(2)}`;
    ...
    <Card className="p-4 shadow-sm hover:shadow-md transition-all duration-200 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
      ...
      <div className="space-y-2 mt-2">
        <div>
          ...
          <Input type="number" step="0.01" value={totalAmount} onChange={(e) => setTotalAmount(parseFloat(e.target.value) || 0)} className={`w-full text-sm ${isReadOnly ? 'bg-gray-100 dark:bg-gray-700/50 cursor-not-allowed opacity-60' : ''}`} disabled={isReadOnly} placeholder="0.00" />
        </div>

        {splitCount > 0 && (
          <div className="text-xs text-gray-500">Split among: {splitCount} tenants <span className="ml-2">Each pays: ${eachPays.toFixed(2)}</span></div>
        )}
