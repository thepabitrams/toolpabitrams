// src/tools/houseandhome/tenant-billing/modules/tenants/TenantDetail/ChargesCard/OneTimeCharge.tsx
...
export const OneTimeCharge: React.FC<OneTimeChargeProps> = ({
  ...
  const handleCopy = async () => {
    const text = `${charge.name}: $${amount.toFixed(2)} (One-Time)`;
    ...
    <Card className="p-4 shadow-sm hover:shadow-md transition-all duration-200 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
      ...
      <div className="space-y-2 mt-2">
        <div>
          ...
          <Input type="number" step="0.01" value={amount} onChange={(e) => setAmount(parseFloat(e.target.value) || 0)} className={`w-full text-sm ${isReadOnly ? 'bg-gray-100 dark:bg-gray-700/50 cursor-not-allowed opacity-60' : ''}`} disabled={isReadOnly} placeholder="0.00" />
        </div>

        {charge.splitAmong && charge.splitAmong.length > 0 && (
          <div className="text-xs text-gray-500">Split among: {charge.splitAmong.length} tenants</div>
        )}
