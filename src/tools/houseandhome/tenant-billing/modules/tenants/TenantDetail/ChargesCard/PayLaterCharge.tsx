// src/tools/houseandhome/tenant-billing/modules/tenants/TenantDetail/ChargesCard/PayLaterCharge.tsx
...
export const PayLaterCharge: React.FC<PayLaterChargeProps> = ({
  charge,
  onUpdate,
  onTogglePaid,
  onEdit,
  ...
  const [startBalance, setStartBalance] = useState(charge.startBalance ?? 0);
  const [decrementAmount, setDecrementAmount] = useState(charge.decrementAmount ?? 0);
  const [note, setNote] = useState(charge.note ?? '');
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    setStartBalance(charge.startBalance ?? 0);
    setDecrementAmount(charge.decrementAmount ?? 0);
    setNote(charge.note ?? '');
  }, [charge]);

  const remainingAmount = Math.max(0, startBalance - decrementAmount);
  const currentAmount = Math.min(decrementAmount, startBalance);
  ...
  }, [startBalance, decrementAmount, note]);
  ...
  const handleCopy = async () => {
    const text = `${charge.name}\nStarting Balance: $${startBalance.toFixed(2)}\nDecrement: $${decrementAmount.toFixed(2)}\nRemaining: $${remainingAmount.toFixed(2)}\nThis Month: $${currentAmount.toFixed(2)}`;
    ...
    <Card className="p-4 shadow-sm hover:shadow-md transition-all duration-200 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-blue-500 bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 rounded-full">
            {getDurationLabel() || 'Pay-Later'}
          </span>
          {isComplete && <span className="text-xs text-gray-500 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full">✓ Completed</span>}
        </div>
      ...
      <div className="space-y-2 mt-2">
        <div className="grid grid-cols-3 gap-2">
          <div>
            <label className="text-xs text-gray-500 block">Starting Balance ($)</label>
            <Input type="number" step="0.01" value={startBalance} onChange={(e) => setStartBalance(parseFloat(e.target.value) || 0)} className={`w-full text-sm ${isReadOnly ? 'bg-gray-100 dark:bg-gray-700/50 cursor-not-allowed opacity-60' : ''}`} disabled={isReadOnly} />
          </div>
          <div>
            <label className="text-xs text-gray-500 block">Decrement ($)</label>
            <Input type="number" step="0.01" value={decrementAmount} onChange={(e) => setDecrementAmount(parseFloat(e.target.value) || 0)} className={`w-full text-sm ${isReadOnly ? 'bg-gray-100 dark:bg-gray-700/50 cursor-not-allowed opacity-60' : ''}`} disabled={isReadOnly} />
          </div>
          <div>
            <label className="text-xs text-gray-500 block">Remaining</label>
            <div className="text-sm font-semibold text-gray-800 dark:text-gray-200 mt-1">${remainingAmount.toFixed(2)}</div>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center mt-1 pt-2 border-t border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-2">
            ...
            {isComplete && <span className="ml-2 text-xs text-green-500">✓ Completed</span>}
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-gray-500">This Month:</span>
            <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">${currentAmount.toFixed(2)}</span>
          </div>
        </div>
      </div>
    </Card>
